'use strict';var Aegis=function(selector){if(typeof selector=="string")this.collection=document.querySelectorAll(selector);else if(typeof selector=="object")if(selector.length>=1)this.collection=selector;else this.collection=[selector]};Aegis.prototype.hide=function(){for(var i=0;i<this.collection.length;i++)this.collection[i].display="none"};Aegis.prototype.show=function(){for(var i=0;i<this.collection.length;i++)this.collection[i].display="block"};
...
Aegis.prototype.find=function(selector){return new Aegis(this.collection[0].querySelectorAll(selector))};Aegis.prototype.attribute=function(attribute,value){if(typeof value==="undefined")return this.collection[0].setAttribute(attribute,value);else this.collection[0].getAttribute(attribute)};function $_(selector){if(typeof selector!="undefined")return new Aegis(selector);else return Aegis}function $_ready(callback){window.addEventListener("load",callback)}
...
Text.removePunctuation=function(text){var special=new Array(";",",",".");for(var character in special)text=text.replace(new RegExp(special[character],"g"),"");return text};var View=function(view,data){this.view=document.querySelector(view);this.viewContent=this.view.innerHTML;this.data=typeof data=="undefined"?null:data};View.prototype.getView=function(){return this.view};View.prototype.isCompilable=function(){return this.data!=null};
View.prototype.compile=function(){if(isCompilable){var compiledView=this.viewContent;for(var i in data)compiledView=compiledView.replace(new RegExp("{{"+i+"}}","g"),data[i]);this.view.innerHTML=compiledView}};View.prototype.show=function(){document.querySelector("[data-view].active").classList.remove("active");this.view.classList.add("active")};
